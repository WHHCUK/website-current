...
export = async function (migration, { makeRequest, spaceId, accessToken }) {
  ...
  settings.createField('footerColumn1Links', {
    ...
    items: {
      type: 'Link',
      validations: [ 
          { 'linkContentType': ['news-article'] }
        ],
        linkType: 'Entry'
      }
  });

  settings.changeFieldControl('footerColumn1Links','builtin','entryLinksEditor', {
    bulkEditing: false, 
  ...
  settings.changeFieldControl('footerColumn2Links','builtin','entryLinksEditor', {
    bulkEditing: false, 
    showCreateEntityAction: false, 
    showLinkEntityAction: true
  });
